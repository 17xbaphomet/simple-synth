    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        ui.horizontal(|ui| {
            ...
            for wave in Waveform::ALL {
                if ui
                    .selectable_label(self.wave_a == wave, wave.label())
                    .clicked()
                {
                    self.wave_a = wave;
                    self.engine
        ...
        ui.horizontal(|ui| {
            ...
            for wave in Waveform::ALL {
                if ui
                    .selectable_label(self.wave_b == wave, wave.label())
                    .clicked()
                {
                    self.wave_b = wave;
                    self.engine
        ...
        if ui
            .add(egui::Slider::new(&mut self.mix, 0.0..=1.0).text("Mix A→B"))
            .changed()
        {
            self.engine
                .lock()
                .unwrap_or_else(|p| p.into_inner())
                .set_osc_mix(self.mix);
        }
        if ui
            .add(egui::Slider::new(&mut self.ratio, 0.25..=4.0).text("Verhältnis B/A"))
            .changed()
        {
            self.engine
                .lock()
                .unwrap_or_else(|p| p.into_inner())
                .set_osc_ratio(self.ratio);
        }
        if ui
            .add(egui::Slider::new(&mut self.detune, -50.0..=50.0).text("Detune B (Cent)"))
            .changed()
        {
            self.engine
                .lock()
                .unwrap_or_else(|p| p.into_inner())
                .set_osc_detune(self.detune);
        }
        if ui
            .add(egui::Slider::new(&mut self.depth, 0.0..=2.0).text("Tiefe AM/FM"))
            .changed()
        {
            self.engine
                .lock()
                .unwrap_or_else(|p| p.into_inner())
                .set_osc_depth(self.depth);
        }
        ...
        ui.horizontal(|ui| {
            ...
            for layout in [Layout::Qwertz, Layout::Qwerty] {
                if ui
                    .selectable_label(self.layout == layout, layout.name())
                    .clicked()
                {
                    self.layout = layout;
                }
        ...
        ui.horizontal(|ui| {
            if ui
                .add(egui::Slider::new(&mut self.noise, 0.0..=0.5).text("Zufall ±"))
                .changed()
            {
                self.engine
                    .lock()
            ...
            for mode in NoiseMode::ALL {
                if ui
                    .selectable_label(self.noise_mode == mode, mode.label())
                    .clicked()
                {
    ...
    fn handle_keyboard(&mut self, ctx: &egui::Context) {
        ...
        ctx.input(|input| {
            for position in all_piano_keys() {
                let typed = self.layout.typed_key(position);
                if input.key_down(typed) {
