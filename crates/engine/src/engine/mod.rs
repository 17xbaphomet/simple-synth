...
impl Engine {
    ...
    pub fn set_sample_rate(&mut self, sample_rate: f32) {
        ...
        if (sr - self.sample_rate).abs() < 0.01 {
            return;
        }
        self.sample_rate = sr;
        self.all_notes_off();
...
#[cfg(test)]
mod tests {
    use super::{Engine, NoiseDist, NoiseMode, VoiceConfig};
    ...
    #[test]
    fn set_sample_rate_resets_voices() {
        let mut engine = Engine::new(VoiceConfig {
        ...
        .unwrap();
        engine.note_on_midi(60);
        engine.set_sample_rate(44_100.0);
        assert!((engine.sample_rate() - 44_100.0).abs() < 0.1);
